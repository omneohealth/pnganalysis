...
```{r, echo = TRUE, eval = TRUE}
temp1 <- aggregate(anc1 ~ ADM1_PCODE + ADM1_EN + month + year, data = mProvince, FUN = sum)
temp2 <- aggregate(sf ~ ADM1_PCODE + ADM1_EN + month + year, data = mProvince, FUN = unique)
...
```{r, echo = TRUE, eval = TRUE, fig.pos = "H", fig.retina = 1}
ggplot(temp1, aes(as.Date(date), anc1Std)) + 
  geom_line() + 
  facet_wrap(vars(ADM1_EN), ncol = 5, nrow = 5, drop = FALSE)
```
